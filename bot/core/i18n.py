...
def i18n_command(command: _CommandT) -> _CommandT:
    ...
    if command.name_localizations is None or not isinstance(command.name_localizations, dict):
    ...
    if isinstance(command, SlashCommand):
        if command.description_localizations is None or not isinstance(command.description_localizations, dict):
        ...
        for option in command.options:
            # logging.getLogger("bot").info(option.description_localizations)
            if option.name_localizations is MISSING:
                option.name_localizations = {}
            elif isinstance(option.name_localizations, TranslatorString):
                option.name_localizations = dict(option.name_localizations)

            if option.description_localizations is MISSING:
                option.description_localizations = {}
            elif isinstance(option.description_localizations, TranslatorString):
                option.description_localizations = dict(
                    option.description_localizations)
