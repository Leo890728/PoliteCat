...
@cog_i18n
class PlayerCog(BaseCog, name="播放器"):
    ...
    @staticmethod
    async def get_voice_client(ctx: Union[ApplicationContext, Interaction, Message], *, ensure: bool = False, send_error_message=True) -> wavelink.Player | None:
        if not ctx.guild:
        ...
        if not player and ensure:
            try:
                # type: ignore
                user: Union[discord.User, discord.Member] = ctx.author if isinstance(ctx, Message) else ctx.user
                player = await user.voice.channel.connect(cls=wavelink.Player)
                await ctx.guild.change_voice_state(
                    channel=user.voice.channel, self_deaf=True)
    ...
    @staticmethod
    async def pause_or_resume(ctx: ApplicationContext) -> bool:
...
@cog_i18n
@discord.guild_only()
class MusicCog(BaseCog, name="音樂"):

    # music = discord.SlashCommandGroup("music", "Music-related commands.")
    ...
    async def play(self, ctx: ApplicationContext, query: str, recommend_queue: bool) -> None:
        ...
        if not hasattr(player, "home"):
            ...
            await ctx.followup.send(f"You can only play songs in {player.home.mention}, as the player has already started there.")
            return
        ...
        if not tracks:
            await ctx.followup.send(f"{ctx.user.mention} - Could not find any tracks with that query. Please try again.")
            return
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            await ctx.followup.send(f"Added the playlist **`{tracks.name}`** ({added} songs) to the queue.")
        else:
            track: wavelink.Playable = tracks[0]
            track.extras = {"from_cog": "MusicCog"}
            await player.queue.put_wait(track)
            await ctx.followup.send(f"Added **`{track}`** to the queue.")
